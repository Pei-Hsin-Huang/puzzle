...
def flatten_image(image, piece_size, indexed=False):
    ...
    """
    rows, columns = int(image.shape[0] // piece_size), int(image.shape[1] // piece_size)
    pieces = []
...
if __name__ == "__main__":
    name = input( 'Please enter picture name(One_Piece.bmp...):' )
    ...
    try:
      ...
      print( "no such img at current dict" )
      exit(0)
    
    image = cv.cvtColor(image, cv.COLOR_BGR2RGB)
    piece_size = 120
 
    print("\n=== Population:  {}".format(144))
    print("=== Generations: {}".format(50))
    print("=== Piece size:  {} px".format(piece_size))

    # Let the games begin! And may the odds be in your favor!
    start = time()
    algorithm = GeneticAlgorithm(image, piece_size, 144, 50)
    solution = algorithm.start_evolution(verbose = True)
    end = time()

    print("\n=== Done in {0:.3f} s".format(end - start))
    solution_image_bgr = solution.to_image()
    solution_image = cv.cvtColor(solution_image_bgr, cv.COLOR_BGR2RGB)
    solution_image_name = name.split(".")[0] + "_result.bmp"

    cv.imwrite(solution_image_name, solution_image)
    print("=== Result saved as '{}'".format(solution_image_name))

    print("=== Close figure to exit")
    show_image(solution_image_bgr, "Solution")
